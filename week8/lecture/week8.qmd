...
filelist <- list.files(here::here("week8", "lecture", "data", "landsat-data"), full.names = TRUE)
...
plotRGB(landsat, r =3, g=2, b=1, stretch = "lin") # stretch linearly to fill from min to max
...
SB_county_south <- st_read(here::here("week8", "lecture","data", "SB_county_south.shp")) %>%
...
# mask the raster to the souther portion of SB county
...
training_data <- st_read(here::here( "week8", "lecture","data", "trainingdata.shp")) %>%
...
```{r}
# Any pixels within the classifications values are creating a row 
# extract the pixel values from landsat raster at the locations in the training data
training_data_values <- terra::extract(landsat, training_data, df = TRUE) # returned a df instead of matrix

# convert training data to data frame
training_data_attribute <- training_data |> 
  st_drop_geometry()

# join training data attributes and extracted reflectance values
SB_training_data <- left_join(training_data_values, training_data_attribute,
                              by =c("ID" = "id")) |> 
...
SB_decision_tree <- rpart(formula = SB_formula,
                          data = SB_training_data, # values are coming from this data
                          method = "class",
                          na.action = na.omit)
```
...
```{r}
# classify the image based on decision tree
SB_classification_dt <- terra::predict(landsat, SB_decision_tree, type= "class", na.rm =TRUE)

# inspect levels to understand the order of classes in prediction
...
# scale the data 
spectral_data_scales <- scale(spectral_data)
```
...
```{r}
# set seed for reproducibitiy
set.seed(223)

# perform k means with k =4
kmeans_result <- kmeans(spectral_data_scales, centers = 4, nstart = 25)
