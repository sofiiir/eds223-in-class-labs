...
template_raster_low <- rast(ext(park_polygon), resolution = .05, 
                            crs = st_crs(park_polygon))

# create high spatial resolution template raster
template_raster_high <- rast(ext(park_polygon), resolution = .01, 
                             crs = st_crs(park_polygon))

# rasterize to low resoltion template raster
...
template_raster <- rast(ext(cycle_hire_osm_projected), resolution = 1000, 
                        crs = st_crs(cycle_hire_osm_projected))
```
...
ch_raster3 <- rasterize(cycle_hire_osm_projected, template_raster, 
                        field = "capacity", # what column we want the function to compute on
                        fun = "sum",
                        na.rm = TRUE)
```
...
# Decide what to use based on what you want to look at 

# true colors correspond channels to the wavelengths
map1 <- tm_shape(L7) +
...
tmap_arrange(map1, map2, ndvi_map, map_ndwi)
```
#### Normalized Difference Moisture Index: vegetation index used in RS to measure and monitor the water content of vegetation
