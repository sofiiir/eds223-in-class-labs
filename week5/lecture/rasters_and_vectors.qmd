...
```{r}
# set all elevation values below 2000 
elevation_masked[elevation_masked <2000] <- NA

# convert high elevation pixels into polygin shapes and then transfrom into a sf
...
bryce = raster(here::here("week5/lecture/data/Bryce_Canyon.tif"))
...
bryce_highway = opq(osm_bbox)  |>  # opq is a query that only looks within the bbox
...
# only pulling out the lines because those are the ones that map trails etc
...
```{r}
# create a combined trails layer with different tiles for each type
# take spatial data and transforming into a raster that can be layered in a map
...
# text is always black so the header has to be a different color 
```
...
# make  a 3D map!  
plot_3d(bryce_zoom_mat, windowsize=c(1200,800)) # how big do you want the rendering 
render_camera(theta=240,  phi=30, zoom=0.3,  fov=60)  
render_snapshot() # a screenshot of the 3D image for the plots pane
```
