...
```{r}
# Map Canterbury adn highest points in NZ
tm_shape(nz) +
  tm_polygons() +
tm_shape(canterbury) +
...
c_height1 <- nz_height[canterbury, ] # Automatically finds the intersection 
```
...
```{r}
# canterbury selects for rows, the space between the commas selects for columns
c_height_disjoint <- nz_height[canterbury, ,op = st_disjoint] 

# Check that intersect is opposite of disjoin 
...
nz_height_logical # True if it intercepts with Canterbury and False if not 

# Filter based on the logical vector
...
```{r}
# Find height within 1000 km of Canterbury
...
# Filter base on logical vector 
cheight_4 <- nz_height[nz_height_logical,] # Select rows then after the comma select columns in this case
```
...
tm_shape(cycle_hire) +
  tm_symbols(fill = "hotpink", fill_alpha = 0.2) +
tm_shape(cycle_hire_osm) +
  tm_symbols(fill = "skyblue", fill_alpha = 0.2)
```
...
map1 <- tm_shape(nz_elevation_agg) +
  tm_polygons(fill = "elevation",
              fill.legend = tm_legend(title = "Mean elevation (meters)")) +
