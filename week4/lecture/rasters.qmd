...
```{r}
# Requires the number of layers for rows to be updates
...
```{r}
# Create a point with an area covered by raster
...
```{r}
# Find the extent of the current raster
...
# Select vlaues that fall withing smaller extent
...
# Subset elevation raster base on mask
...
rcl <- matrix(c(1000, 1500, 1, # Group 1 ranges from 1000 to 1500 m
...
# Use reclss matrix to reclassify elevation raster
reclassified <- classify(zion_elevation, rcl = rcl)

# Change reclassified values into factors 
#  Factors are  the groups we made in the matrix above(1, 2, 3, 4)
...
# Can also apply to 'landsat4', 'landsat3' in place of 4, 3
# Order of the landsats is decided based on the order of the function 
...
```{r}
# Getting the minimum value for every 9x9 box and keeping the min value
...
```{r}
zion_elevation_course <- aggregate(zion_elevation, fact = 5, fun = mean)
```
...
map2 <- tm_shape(zion_elevation_course) +
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) +
  tm_title(text = "Aggregated")
...
zion_elevation_disagg <- disagg(zion_elevation_course,
                                fact =5, # from 1 pixel to a 5x5
                                method = "bilinear")

map3 <- tm_shape(zion_elevation_disagg) +
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) +
  tm_title(text = "Disaggrgated")
