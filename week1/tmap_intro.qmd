...
```{r}
# It plots in the order that they're listed in
tm_shape(world_vector) + # Defines the spatial object to plot
  tm_polygons()    +     # Defines how to plot the object
...
tm_shape(world_elevation) + # Plot global elevations
  ...
  tm_dots() +
  tm_text("name", size = 0.5, col = "blue")
```
...
```{r}
# First variable defines the spatial extent
tm_shape(world_cities) +
  tm_dots() +
  tm_text("name") +
...
```{r}
tm_shape(world_elevation, bbox = c(-15, 35, 45, 65)) + # Add bounding box to restrict extent map (lat,long(xmin, xmax, ymin, ymax))
  tm_raster() +
tm_shape(world_vector) +
  tm_borders()
...
```{r}
# Plot just the borders in red
tm_shape(world_vector) +
  tm_borders(col = "red")
```

```{r}
# Plot a fill polygon with fixed colors
tm_shape(world_vector) + 
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              fill.scale = tm_scale(values = c("lightyellow", "red")),
              fill.legend = tm_legend(title = "Life Expectancy (years)", 
                                      position = tm_pos_out("left"))) # Moving the legend to the outside on the left
...
tm_shape(world_vector) +
  tm_fill() +
  tm_scale_bar() +
  tm_compass(position = c("left", "top")) # "north = 180" can be added to change which way north is facing
``` 
